...
- `children::NTuple{D,RefValue{AbstractNode}}`: Children of the current node.
    The children need only be defined up to `degree`,
    otherwise, leave it undefined (see the
...
- `op::UInt8`: If `degree==1`, this is the index of the operator
    in `operators.ops[1]`. If `degree==2`, this is the index of the
    operator in `operators.ops[2]`. In other words, this is an enum
...
for N in (:Node, :GraphNode)
    @eval mutable struct $N{T,D} <: AbstractExpressionNode{T,D}
        ...
        op::UInt8  # (possibly undefined) If operator, this is the index of the operator in operators.binops, or operators.unaops
        children::NTuple{D,Base.RefValue{AbstractNode}}
...
- `op::UInt8`: If `degree==1`, this is the index of the operator
    ...
    of the operators, and is dependent on the specific `OperatorEnum`
    object. Only defined if `degree >= 1`
- `children::NTuple{D,Node{T}}`: Children of the node. Only defined up to `degree`.

# Constructors

    Node([T]; val=nothing, feature=nothing, op=nothing, l=nothing, r=nothing, children=nothing, allocator=default_allocator)
    Node{T,D}(; val=nothing, feature=nothing, op=nothing, l=nothing, r=nothing, children=nothing, allocator=default_allocator)
