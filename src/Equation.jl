...
# Constructors
    ...
    Node([T]; val=nothing, feature=nothing, op=nothing, l=nothing, r=nothing)
    Node{T}(; val=nothing, feature=nothing, op=nothing, l=nothing, r=nothing)
...
struct Node{T} <: AbstractExpressionNode{T}
    ...
    val::Ref{T}  # If is a constant, this stores the actual value
...
function Base.setproperty!(n::AbstractExpressionNode, name::Symbol, value)
    if name == :l || name == :r || name == :val
        getfield(n, name)[] = value
    else
        setfield!(n, name, value)
    end
end
...
@inline function (::Type{N})(
    ::Type{T1}=Undefined; val=nothing, feature=nothing, op=nothing, l=nothing, r=nothing,
) where {T1,N<:AbstractExpressionNode}
    return node_factory(N, T1, val, feature, op, l, r)
end

"""Create a constant leaf."""
@inline function node_factory(
    ::Type{N}, ::Type{T1}, val::T2, ::Nothing, ::Nothing, ::Nothing, ::Nothing
) where {N,T1,T2}
    T = node_factory_type(N, T1, T2)
    return with_type_parameters(N, T)(0, false, Ref(convert(T, val)), 0, 0, Ref{Node{T}}(), Ref{Node{T}}())
end

"""Create a variable leaf, to store data."""
@inline function node_factory(
    ::Type{N}, ::Type{T1}, ::Nothing, feature::Integer, ::Nothing, ::Nothing, ::Nothing,
) where {N,T1}
    T = node_factory_type(N, T1, DEFAULT_NODE_TYPE)
    return with_type_parameters(N, T)(0, false, Ref{T}(), feature, 0, Ref{Node{T}}(), Ref{Node{T}}())
end

"""Create a unary operator node."""
@inline function node_factory(
    ::Type{N}, ::Type{T1}, ::Nothing, ::Nothing, op::Integer, l::AbstractExpressionNode{T2}, ::Nothing,
) where {N,T1,T2}
    @assert l isa N
    T = T2  # Always prefer existing nodes, so we don't mess up references from conversion
    NT = with_type_parameters(N, T)
    l = typeof(l) === NT ? l : convert(NT, l)
    return with_type_parameters(N, T)(1, false, Ref{T}(), 0, op, Ref(l), Ref{Node{T}}())
end

"""Create a binary operator node."""
@inline function node_factory(
    ::Type{N}, ::Type{T1}, ::Nothing, ::Nothing, op::Integer, l::AbstractExpressionNode{T2}, r::AbstractExpressionNode{T3},
) where {N,T1,T2,T3}
    T = promote_type(T2, T3)
    NT = with_type_parameters(N, T)
    l = typeof(l) === NT ? l : convert(NT, l)
    r = typeof(r) === NT ? r : convert(NT, r)
    return NT(2, false, Ref{T}(), 0, op, Ref(l), Ref(r))
end
