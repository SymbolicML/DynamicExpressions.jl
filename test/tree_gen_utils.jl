import DynamicExpressions:
    Node, NodeSampler, copy_node, count_nodes, has_constants, has_operators
...
function append_random_op(
    ...
    choice = rand()
    make_new_bin_op =
        make_new_bin_op === nothing ? choice < nbin / (nuna + nbin) : make_new_bin_op

    new_node = if make_new_bin_op
        Node(rand(1:nbin), make_random_leaf(nfeatures, T), make_random_leaf(nfeatures, T))
    else
    ...
    function has_child_leaf(t)
        return (t.degree > 0 && t.l.degree == 0) || (t.degree > 1 && t.r.degree == 0)
    end
