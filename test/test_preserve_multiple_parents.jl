...
copy_with_topology
@test length(collect(eachmatch(r"exp", string_tree(copy_with_topology, operators)))) == 2
@test copy_with_topology.l.l === copy_with_topology.r
@test hash(copy_with_topology.l.l) == hash(copy_with_topology.r)
@test string_tree(copy_with_topology.l.l, operators) != string_tree(base_tree, operators)
