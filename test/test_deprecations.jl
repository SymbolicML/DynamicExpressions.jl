using Test, DynamicExpressions, Zygote, LoopVectorization
using Suppressor: @capture_err
using DispatchDoctor: allow_unstable

operators = OperatorEnum(; binary_operators=[+, -, *, /], unary_operators=[cos, sin])
x1, x2 = NNode{Float64}(; feature=1), NNode{Float64}(; feature=2)
tree = cos(2.1 * x1)

# Also test warnings:
for constructor in (OperatorEnum, GenericOperatorEnum)
    VERSION >= v"1.9" &&
        @test_logs (:warn, r"The `tree\(X; kws...\)` syntax is deprecated.*") tree(
            [1.0; 2.0;;]
        )

    constructor == GenericOperatorEnum && continue

    if VERSION >= v"1.9"
        @test_logs (:warn, r"The `tree'\(X; kws...\)` syntax is deprecated.*") allow_unstable() do
            tree'([1.0; 2.0;;])
        end
    end
end

if VERSION >= v"1.9"
    @test_logs (:warn, r"NNode\(d, c, v\) is deprecated.*") (
        n = NNode(1, true, 1.0 + 0im); @assert (n.val isa ComplexF64)
    )
    @test_logs (:warn, r"NNode\(T, d, c, v\) is deprecated.*") (
        n = NNode(Float32, 1, true, 1.0 + 0im); @assert (n.val isa Float32)
    )
    @test_logs (:warn, r"NNode\(T, d, c, v, f\) is deprecated.*") (
        n = NNode(Float32, 1, false, nothing, 1); @assert (n.feature == 1)
    )
    @test_logs (:warn, r"NNode\(d, c, v, f, o, l\) is deprecated.*") (
        x1 = NNode(; feature=1);
        n = NNode(1, true, nothing, 1, 3, x1);
        @assert (n.op == 3 && n.l === x1)
    )
    @test_logs (:warn, r"NNode\(d, c, v, f, o, l, r\) is deprecated.*") (
        x1 = NNode(; feature=1);
        x2 = NNode(; feature=2);
        n = NNode(2, true, nothing, 1, 1, x1, x2);
        @assert (n.op == 1 && n.l === x1 && n.r === x2)
    )
end

# Test deprecated modules
logs = @capture_err begin
    @eval using DynamicExpressions.EquationModule
end
@test contains(logs, "DynamicExpressions.EquationModule is deprecated,")

DynamicExpressions.EquationModule.NNode === DynamicExpressions.NodeModule.NNode
